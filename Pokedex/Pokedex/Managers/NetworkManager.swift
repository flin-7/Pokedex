...
class NetworkManager {
    ...
    func getPokemons(offset: Int, completed: @escaping (Result<Pokemons, PDError>) -> Void) {
        let endpoint = baseURL + "/pokemon?limit=100&offset=\(offset)"
    ...
    func getPokemonFlavorText(pokemonIndex: String, completed: @escaping (Result<PokemonSpecies, PDError>) -> Void) {
        let endpoint = baseURL + "/pokemon-species/\(pokemonIndex)"
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            do {
                let decoder = JSONDecoder()
                decoder.keyDecodingStrategy = .convertFromSnakeCase
                let pokemonSpeciesInfo = try decoder.decode(PokemonSpecies.self, from: data)
